...
static char *update_track(char *track, char *save, unsigned int pos)
{
	if (pos > 0)
		track[pos - 1] = *save;
	*save = track[pos];
...
static char *cross_bridge(char *track, int sem_id, struct sembuf *sem)
{
	char save_char = 0;
	unsigned int pos = 0;
